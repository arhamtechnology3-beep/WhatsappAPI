...
1. In wacrm go to **Settings → Templates** and click **Install recipes**. That creates local **DRAFT** `*_v2` rows (it does not re-seed on every page load). It also points Shopify automations at those names and drops leftover `*_v1` drafts.
2. Open each **v2** draft → **Submit for Approval**. Do not resubmit `*_v1` — Meta already has English (US) for those names (and some were created as MARKETING, so UTILITY resubmit fails).
3. Template **names must match exactly** (e.g. `wacrm_cart_abandoned_v2`). Language: `en_US`.
...
| `wacrm_cart_abandoned_v2` | MARKETING | 20 min | `{{1}}` name, `{{2}}` items | Complete Purchase (checkout), Shop From WhatsApp |
